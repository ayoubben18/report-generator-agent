...
import WorkflowApproval, {
  ...
  Chapter,
  Section,
} from "@/app/components/WorkflowApproval";
...
export function AnimatedAIChat() {
  ...
  const addIdsToSections = (plan: any): Plan => {
    return {
      ...plan,
      chapters: plan.chapters.map((chapter: any) => ({
        ...chapter,
        sections: chapter.sections.map((section: any, index: number) => ({
          ...section,
          id:
            section.id ||
    ...
    try {
      ...
      if (data.status === "suspended") {
        setWorkflowState({
          status: "suspended",
          runId: data.runId,
          generatedPlan: addIdsToSections(data.generatedPlan),
  ...
  const handleSendMessage = () => {
    if (value.trim()) {
      // Check if it's a report generation command or regular message
      if (value.startsWith("/report") || workflowState.status === "idle") {
        startWorkflow();
      } else {
        // Handle other commands or regular chat
        startTransition(() => {
          setIsTyping(true);
          setTimeout(() => {
            setIsTyping(false);
  ...
  const selectCommandSuggestion = (index: number) => {
    ...
    if (selectedCommand.prefix === "/report") {
      setValue("Describe what kind of report you want to generate...");
      setShowCommandPalette(false);
