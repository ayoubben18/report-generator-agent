...
export default function WorkflowApproval({
  ...
  const editedPlan = addIdsToSections(generatedPlan);
  ...
  const handleApprove = () => {
    ...
    onApprove({
      ...
      modifiedPlan: isEditing ? modifiedPlan : generatedPlan,
    });
  };
  ...
  const handleReject = () => {
    ...
    onApprove({
      approved: false,
      feedback: formData.feedback || "User rejected the plan",
    });
  };

  const updateChapterTitle = (chapterIndex: number, newTitle: string) => {
    setValue(`chapters.${chapterIndex}.title`, newTitle);
  };
  ...
  const updateChapterDescription = (
    ...
    setValue(`chapters.${chapterIndex}.description`, newDescription);
  };
  ...
  const updateSectionTitle = (
    ...
    const currentChapters = getValues("chapters");
    const sectionIndex = currentChapters[chapterIndex].sections.findIndex(
      (s) => s.id === sectionId
    );
    if (sectionIndex !== -1) {
      setValue(
  ...
  const updateSectionDescription = (
    ...
    if (sectionIndex !== -1) {
      setValue(
        `chapters.${chapterIndex}.sections.${sectionIndex}.description`,
        newDescription
      );
    }
  };
  ...
  const removeSection = (chapterIndex: number, sectionId: string) => {
    const currentChapters = getValues("chapters");
    const updatedChapters = [...currentChapters];
    const sectionIndex = updatedChapters[chapterIndex].sections.findIndex(
      (s) => s.id === sectionId
    );
    if (sectionIndex !== -1) {
  ...
  const reorderSections = (
    ...
    setValue(`chapters.${chapterIndex}.sections`, newSectionOrder);
  };
      ...
      >
        ...
        >
          ...
          <div className="space-y-6">
            {watch("chapters").map((chapter, chapterIndex) => (
              <motion.div
                key={chapterIndex}
              ...
              <FormItem>
                <FormControl>
                  <Textarea
                    ...
                    rows={4}
                    className={cn(
                      "w-full px-4 py-3 bg-white/[0.05] border border-white/[0.1] rounded-xl text-white/90 placeholder:text-white/30 focus:outline-none focus:border-violet-400/50 transition-colors resize-none"
                    )}
                  />
                </FormControl>
